...
function createMockContainer(tokens: string[]): string {
  const containerBytes = cborg.encode({
    'ctn-v1': tokens.map(tokenToBytes),
  })

  return `${String.fromCharCode(0x43)}${bytesToBase64URL(containerBytes)}`
}

export const MOCK_CONTAINER_TOKEN = createMockContainer([MOCK_DELEGATION_TOKEN, MOCK_INVOCATION_TOKEN])
...
export const TOKEN_SCENARIOS: MockTokenScenario[] = [
  ...
  {
    name: 'UCAN Container Token',
    description: 'Authorization header carrying a UCAN container with a delegation and an invocation token',
    authorizationHeader: `Bearer ${MOCK_CONTAINER_TOKEN}`,
    expectedType: 'delegation',
    expectedVersion: '1.0.0-rc.1',
  },
  {
    name: 'UCAN Container in ucans Header',
    description: 'ucans header with a UCAN container token alongside a raw delegation token',
    ucansHeader: `${MOCK_CONTAINER_TOKEN}, ${MOCK_DELEGATION_TOKEN}`,
    expectedType: 'delegation',
    expectedVersion: '1.0.0-rc.1',
  },
  {
  ...
  },
  {
    ...
    authorizationHeader: `Bearer ${bytesToBase64(new TextEncoder().encode('random data here'))}`,
    expectedType: 'unknown',
  },
]
